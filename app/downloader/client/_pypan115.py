...
class PyPan115:
    ...
    def getdirid(self, tdir):
        try:
            ...
            log.exception(f"【PyPan115】获取目录ID异常：")
            self.err = "异常错误：{}".format(result)
        return False, ''
    ...
    def gettasklist(self, page=1):
        try:
            ...
            log.exception(f"【PyPan115】获取任务列表异常：")
            self.err = "异常错误：{}".format(result)
        return False, []
        ...
        try:
            ...
            if re.match("^https*://", content):
                try:
                    ...
                    log.exception(f"【PyPan115】HTTP重定向异常：")
            ...
            log.exception(f"【PyPan115】添加任务异常：")
            self.err = "异常错误：{}".format(result)
        return False, ''
    ...
    def deltask(self, thash):
        try:
            ...
            log.exception(f"【PyPan115】删除任务异常：")
            self.err = "异常错误：{}".format(result)
        return False

    # 根据ID获取文件夹路径 / Get folder path by ID
    def getiddir(self, tid):
        try:
            ...
            log.exception(f"【PyPan115】获取ID路径异常：")
            self.err = "异常错误：{}".format(result)